...
extension ComplexMatrix where Scalar == Double {
    ...
    public func frft1D(a: Scalar, setup: FFT<Scalar>.Setup? = nil) -> ComplexMatrix<Scalar> {
        ...
        let inputChirp = ComplexMatrix.frftInputChirp(shape: .row(length: shape.length), a: a)
        let outputChirp = ComplexMatrix.frftOutputChirp(shape: .row(length: shape.length), a: a)
