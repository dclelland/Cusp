...
extension DFrFT where Scalar == Float {
...
extension DFrFT where Scalar == Float {
    ...
    internal func finiteDifferenceCoefficients(derivativeOrder: Int) -> Matrix<Scalar> {
        return .init(shape: .row(length: derivativeOrder + 1)) { row, column in
            let index = column
            let sign = Scalar(index % 2 == 0 ? 1.0 : -1.0)
            let coefficient = tgammaf(Scalar(derivativeOrder + 1)) / (tgammaf(Scalar(index + 1)) * tgammaf(Scalar(derivativeOrder - index + 1)))
